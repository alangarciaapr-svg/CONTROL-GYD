...
def page_sgsst(
    *,
    ...
    # Stable key prefix — unique per tenant, stable across reruns
    _ck_prefix = str(current_segav_client_key() or "x")[:10].replace(" ", "_")
    def K(name: str) -> str:
        return f"sgsst_{_ck_prefix}_{name}"
    ...
    if stats["matriz_pendiente"] == 0 and int(fetch_value("SELECT COUNT(*) FROM sgsst_matriz_legal", default=0) or 0) > 0:
        _modules_ok += 1
    if stats["programa_abierto"] == 0 and int(fetch_value("SELECT COUNT(*) FROM sgsst_programa_anual", default=0) or 0) > 0:
        _modules_ok += 1
    if stats["miper_criticos"] == 0:
        _modules_ok += 1
    if stats["ds594_abierto"] == 0 and int(fetch_value("SELECT COUNT(*) FROM sgsst_inspecciones", default=0) or 0) > 0:
        _modules_ok += 1
    if stats["incidentes_abiertos"] == 0:
        _modules_ok += 1
    if stats["cap_vencidas"] == 0 and int(fetch_value("SELECT COUNT(*) FROM sgsst_capacitaciones", default=0) or 0) > 0:
        _modules_ok += 1
        ...
        with a2:
            ...
            if df_matriz.empty:
                ...
                if st.button("Recargar base legal", key=K("sgsst_seed_matriz")):
                    ...
                    st.rerun()
                st.divider()
                st.markdown("#### 🗑️ Eliminar requisito")
                _del_mid = st.selectbox("Requisito a eliminar", matriz_ids, format_func=lambda x: f"#{int(x)} · {df_matriz[df_matriz['id']==x].iloc[0]['tema']}", key=K("sgsst_del_matriz_id"))
                st.warning("Esta acción es irreversible.")
                if st.button("Eliminar requisito", key=K("sgsst_del_matriz")):
                    execute("DELETE FROM sgsst_matriz_legal WHERE id=?", (int(_del_mid),))
                    sgsst_log("Matriz legal", "Eliminar", f"#{_del_mid}")
    ...
    with tabs[6]:
        ...
        if df_miper is not None and not df_miper.empty:
            with st.expander("✏️ Editar / 🗑️ Eliminar riesgo"):
                _miper_ids = df_miper["id"].tolist()
                _sel_mid = st.selectbox("Riesgo", _miper_ids, format_func=lambda x: f"#{int(x)} · {str(df_miper[df_miper['id']==x].iloc[0].get('peligro',''))[:50]}", key=K("miper_edit_id"))
                _curr_est = str(df_miper[df_miper['id'] == _sel_mid].iloc[0].get('estado', 'PENDIENTE'))
                _new_est = st.selectbox("Nuevo estado", SGSST_ESTADOS, index=SGSST_ESTADOS.index(_curr_est) if _curr_est in SGSST_ESTADOS else 0, key=K("miper_edit_est"))
                _me1, _me2 = st.columns(2)
                with _me1:
                    if st.button("Actualizar estado", key=K("miper_edit_save")):
                        execute("UPDATE sgsst_miper SET estado=?, updated_at=? WHERE id=?", (_new_est, datetime.now().isoformat(timespec='seconds'), int(_sel_mid)))
                        sgsst_log("MIPER", "Editar", f"#{_sel_mid} → {_new_est}")
                        st.success("Estado actualizado.")
                        st.rerun()
                with _me2:
                    if st.button("🗑️ Eliminar riesgo", key=K("miper_del")):
                        execute("DELETE FROM sgsst_miper WHERE id=?", (int(_sel_mid),))
    ...
    with tabs[9]:
        ...
        if df_inc is not None and not df_inc.empty:
            with st.expander("✏️ Editar / 🗑️ Eliminar incidente"):
                _inc_ids = df_inc["id"].tolist()
                _sel_iid = st.selectbox("Incidente", _inc_ids, format_func=lambda x: f"#{int(x)} · {df_inc[df_inc['id']==x].iloc[0].get('tipo','')} - {df_inc[df_inc['id']==x].iloc[0].get('fecha','')}", key=K("inc_edit_id"))
                _curr_iest = str(df_inc[df_inc['id'] == _sel_iid].iloc[0].get('estado', 'PENDIENTE'))
                _new_iest = st.selectbox("Nuevo estado", SGSST_ESTADOS, index=SGSST_ESTADOS.index(_curr_iest) if _curr_iest in SGSST_ESTADOS else 0, key=K("inc_edit_est"))
                _ie1, _ie2 = st.columns(2)
                with _ie1:
                    if st.button("Actualizar estado", key=K("inc_edit_save")):
                        execute("UPDATE sgsst_incidentes SET estado=?, updated_at=? WHERE id=?", (_new_iest, datetime.now().isoformat(timespec='seconds'), int(_sel_iid)))
                        sgsst_log("Incidentes", "Editar", f"#{_sel_iid} → {_new_iest}")
                        st.success("Actualizado.")
                        st.rerun()
                with _ie2:
                    if st.button("🗑️ Eliminar incidente", key=K("inc_del")):
                        execute("DELETE FROM sgsst_incidentes WHERE id=?", (int(_sel_iid),))
                        sgsst_log("Incidentes", "Eliminar", f"#{_sel_iid}")
