...
def auth_gate_ui():
    """Pantalla de inicio compacta, profesional y sin scroll."""
    ...
    st.markdown(
        ...
        .segav-login-shell{max-width:1280px;margin:0 auto;}
        .segav-login-card{
        ...
        .segav-login-forgot{text-align:center;margin:0.6rem 0 1rem 0;color:#1d5fcc;font-size:1rem;font-weight:500;}
        .segav-login-logo{margin-top:1.45rem;text-align:center;}
        .segav-login-logo-text{font-size:1.15rem;font-weight:800;letter-spacing:0.02em;color:#175ab8;text-align:center;margin-top:0.2rem;}
        .segav-login-divider{width:82%;margin:0.8rem auto 0.15rem auto;border-top:2px solid rgba(23,90,184,0.30);}
        .segav-login-right{
            background:linear-gradient(180deg,#0f57bb 0%,#0a4ea8 100%);
            min-height:78vh;
            display:flex;
            align-items:center;
            justify-content:center;
        ...
        .segav-login-right img{width:100%;height:100%;object-fit:cover;display:block;}
        div[data-testid="stTextInput"] label p{font-size:1.02rem !important;font-weight:700 !important;color:#1f2937 !important;}
        div[data-testid="stTextInput"] input{height:3.2rem !important;border-radius:12px !important;font-size:1.02rem !important;}
        div.stButton > button, div[data-testid="stFormSubmitButton"] > button{height:3.3rem !important;border-radius:12px !important;font-size:1.18rem !important;font-weight:700 !important;box-shadow:0 8px 18px rgba(29,95,204,0.18);}
        .segav-login-help{margin-top:0.6rem;text-align:center;color:rgba(15,23,42,0.68);font-size:0.92rem;}
        @media (max-width:1100px){
            .segav-login-card{min-height:auto;}
            .segav-login-left,.segav-login-right{min-height:auto;}
            .segav-login-left{padding:2.1rem 1.4rem 1.6rem 1.4rem;}
            .segav-login-title{font-size:2rem;text-align:center;}
            .segav-login-right{display:none;}
        }
        </style>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown('<div class="segav-login-shell">', unsafe_allow_html=True)
    left_col, right_col = st.columns([0.88, 1.32], gap="large")
    ensure_users_table()
    ...
    with left_col:
        st.markdown('<div class="segav-login-card"><div class="segav-login-left">', unsafe_allow_html=True)
        st.markdown('<div class="segav-login-title">Acceso al Sistema</div>', unsafe_allow_html=True)

        with st.form("form_login"):
            username = st.text_input("Usuario", placeholder="Ingrese su usuario")
            password = st.text_input("Contraseña", type="password", placeholder="Ingrese su contraseña")
            st.markdown('<div class="segav-login-forgot">¿Olvidó su contraseña?</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="segav-login-logo">', unsafe_allow_html=True)
        if logo_bytes:
            st.image(logo_bytes, width=230)
        else:
            st.markdown("### SEGAV")
        st.markdown('<div class="segav-login-divider"></div><div class="segav-login-logo-text">SEGAV ERP</div></div>', unsafe_allow_html=True)
        st.markdown('<div class="segav-login-help">Si olvidaste tu contraseña, solicita reinicio a un administrador.</div>', unsafe_allow_html=True)
        st.markdown('</div></div>', unsafe_allow_html=True)

    with right_col:
        st.markdown('<div class="segav-login-card segav-login-right">', unsafe_allow_html=True)
        hero_path = os.path.join("assets", "branding", "login_hero_final.png")
        fallback_hero = os.path.join("assets", "branding", "login_hero_segav.svg")
        try:
            if os.path.exists(hero_path):
                st.image(hero_path, use_container_width=True)
            elif os.path.exists(fallback_hero):
                st.image(fallback_hero, use_container_width=True)
            else:
                st.markdown("<div style='padding:2rem;color:white;font-size:2rem;font-weight:800;'>SEGAV ERP</div>", unsafe_allow_html=True)
        except Exception:
            st.markdown("<div style='padding:2rem;color:white;font-size:2rem;font-weight:800;'>SEGAV ERP</div>", unsafe_allow_html=True)
