...
def current_segav_client_key() -> str:
    ...
    if session_key:
        return session_key
    cfg_key = str(segav_erp_value('current_client_key', '') or '').strip()
    if cfg_key:
        return cfg_key
    legacy_key = resolve_legacy_owner_client_key()
    if legacy_key:
        return legacy_key
    return ''
...
def auth_gate_ui():
    """Pantalla de acceso corporativa aprobada, en una sola vista y sin scroll normal en escritorio."""
    ensure_users_table()
    ...
    approved_panel = os.path.join(os.path.dirname(__file__), "assets", "branding", "login_hero_approved.png")
    login_logo = os.path.join(os.path.dirname(__file__), "assets", "branding", "segav_logo.png")

    st.markdown(
        """
        <style>
        header[data-testid="stHeader"], div[data-testid="stToolbar"], .stDeployButton, section[data-testid="stSidebar"] {display:none !important;}
        [data-testid="stAppViewContainer"] > .main {padding-top: 0 !important;}
        .block-container {padding-top: 0 !important; padding-bottom: 0 !important; max-width: 100% !important;}
        html, body, .stApp {height: 100vh; overflow: hidden; background: #f3f4f6;}
        .segav-shell {
            min-height: 100vh;
            display: flex;
            align-items: center;
        ...
        @media (max-width: 980px) {
            ...
            .segav-shell {padding: 10px; min-height: auto;}
            .segav-card {height: auto; grid-template-columns: 1fr;}
            .segav-right {display: none;}
            .segav-left {padding: 28px 22px 24px 22px;}
            .segav-title {font-size: 28px; margin-bottom: 22px;}
        }
        </style>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown('<div class="segav-shell"><div class="segav-card">', unsafe_allow_html=True)
    left, right = st.columns([0.92, 1.38], gap="small")

    with left:
        st.markdown('<div class="segav-left">', unsafe_allow_html=True)
        st.markdown('<div class="segav-title">Acceso al Sistema</div>', unsafe_allow_html=True)
        with st.form("form_login"):
            st.markdown("**Usuario**")
            username = st.text_input("Usuario", label_visibility="collapsed", placeholder="Ingrese su usuario")
            st.markdown("**Contraseña**")
            password = st.text_input("Contraseña", label_visibility="collapsed", type="password", placeholder="Ingrese su contraseña")
        ...
        logo_html = ""
        try:
            if os.path.exists(login_logo):
                with open(login_logo, "rb") as fp:
                    logo_b64 = base64.b64encode(fp.read()).decode("utf-8")
                logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="width:190px;max-width:62%;height:auto;" />'
        except Exception:
            logo_html = ""
        st.markdown(f'<div class="segav-logo-wrap">{logo_html}<div class="segav-logo-text">SEGAV ERP</div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with right:
        st.markdown('<div class="segav-right">', unsafe_allow_html=True)
        if os.path.exists(approved_panel):
            st.image(approved_panel, use_container_width=True)
        else:
            st.markdown("<div style='padding:40px;color:white;font-size:28px;font-weight:800;'>SEGAV ERP</div>", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
