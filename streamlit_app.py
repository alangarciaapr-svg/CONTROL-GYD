...
def _page_export_zip_impl():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["✅ Pendientes", "📦 Generar ZIP", "🗂️ Historial", "📅 Exportar por mes"])
    ...
    with tab2:
        ...
        with colx1:
            if st.button("Generar ZIP y guardar en historial", type="primary", use_container_width=True):
                try:
                    zip_bytes, name, _inc, _skip, _skip_names = export_zip_for_faena(
                        int(faena_id),
                    ...
                    if _skip > 0:
                        st.warning(f"⚠️ {_skip} documento(s) no pudieron incluirse: {', '.join(_skip_names[:10])}")
